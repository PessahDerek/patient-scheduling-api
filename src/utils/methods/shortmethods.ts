...
export const isValidPhoneNumber = (str: string): boolean => {
  ...
  // Check starts with 0
  if (!str.startsWith("0"))
    throw new AppError("Phone number must start with 0", 400);
...
export const convertToDateTime = (time: Date) => {
  const useThis = new Date().getHours();
...
export const handleError = (
  ...
  message: string = "Sorry something went wrong! Please try again!",
) => {
