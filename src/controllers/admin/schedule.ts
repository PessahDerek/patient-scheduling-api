...
export const getSchedule: Controller = async (req, res) => {
    try {
        const schedules = await api.schedules.find({}, {populate: ['doctor', 'workingDay']})
        res.status(200).json(schedules);
    } catch (error) {
        if (error instanceof AppError)
            return res.status(error.status).json({message: error.message})
        console.log("\t", error)
        return res.status(500).json({message: "Sorry, something went wrong while fetching schedule!"})
    }
}
...
export const getWorkingDays: Controller = async (req, res) => {
    try {
        ...
        if (error instanceof AppError)
            return res.status(error.status).json({message: error.message})
        console.log("\t", error)
        return res.status(500).json({message: "Sorry, something went wrong while fetching working days!"})
    }
}
...
export const addWorkingDay: Controller = async (req, res, next) => {
    try {
        ...
        await api.em.persistAndFlush(newWorkingDay)
            .then(() => {
                res.status(200).json({message: "Working day added successfully."}).end()
            })
            .catch(err => {
                console.log(err)
            })
        ...
        console.log("\tError: ", e)
        res.status(500).json({message: "Sorry, something went wrong adding working day!"}).end()
    }
}
...
export const scheduleDoctor: Controller = async (req, res) => {
    try {
        ...
        if (!day)
            return res.status(400).json({message: "Selected day is not a recognized working day!"}).end()
        ...
        if (foundSchedule)
            return res.status(400).json({message: `Doctor ${doctor.firstName} is scheduled at this time already`})
        ...
        await api.em.persistAndFlush(newSchedule)
            .then(() => res.status(200).json({message: `Dr.${doctor.firstName}'s schedule added successfully.`}).end())
            .catch(err => {
                throw err
            })
    } catch (e) {
        if (e instanceof AppError)
            return res.status(e.status).json({message: e.message}).end()
        console.log("\tError: ", e)
        res.status(500).json({message: "Sorry, something went wrong adding schedule!"}).end()
    }
}
...
export const editDoctorSchedule: Controller = async (req, res) => {
    try {
        ...
        handleError(err, res, "Sorry, something went wrong editing doctor's schedule!")
        // if (err instanceof AppError)
        //     return res.status(err.status).json({message: err.message}).end()
        // console.log("\tError: ", err)
        // res.status(500).json({message: "Sorry, something went wrong editing doctor's schedule!"}).end()
    }
}
